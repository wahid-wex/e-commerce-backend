...
func createTables(db *gorm.DB) {
	...
	tables = addNewTable(db, models.ProductReaction{}, tables)
	tables = addNewTable(db, models.ProductQuestion{}, tables)
	tables = addNewTable(db, models.ProductAnswer{}, tables)
...
func createProducts(db *gorm.DB) {
	...
	for _, p := range products {
		var count int64
		db.Model(&models.Product{}).Where("name = ? AND seller_id = ?", p.Name, p.SellerID).Count(&count)
