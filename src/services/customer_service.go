...
const CustomerFilterExp string = "username = ?"
...
func (s *CustomerService) LoginByUsername(req *dto.LoginByUsernameRequest) (*dto.TokenDetail, error) {
	...
	err := s.database.
		...
		Where(CustomerFilterExp, req.Username).
...
func (s *CustomerService) RegisterLoginByMobileNumber(req *dto.RegisterLoginByMobileRequest) (*dto.TokenDetail, error) {
	...
	u := models.Customer{Phone: req.MobileNumber, Username: req.MobileNumber}
...
func (s *CustomerService) loginByMobileNumber(username string) (*dto.TokenDetail, error) {
	var customer models.Customer
	err := s.database.
		Model(&models.Customer{}).
		Where(CustomerFilterExp, username).
		Preload("UserRoles.Role").
		Find(&customer).Error
	if err != nil {
		return nil, err
...
func (s *CustomerService) getDefaultRole() (roleId uint, err error) {
