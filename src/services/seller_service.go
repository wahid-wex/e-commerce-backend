...
const SellerFilterExp string = "username = ?"
const countExp string = "count(*) > 0"
...
func (s *SellerService) LoginSellerByUsername(req *dto.LoginByUsernameRequest) (*dto.TokenDetail, error) {
	...
	err := s.database.
		Model(&models.Seller{}).
		Where(SellerFilterExp, req.Username).
	...
	tokenDto := tokenDto{UserId: seller.ID, Name: seller.StoreName,
		Email: seller.Email, MobileNumber: seller.Phone}
...
func (s *SellerService) loginSellerByMobileNumber(username string) (*dto.TokenDetail, error) {
	var seller models.Seller
	err := s.database.
		Model(&models.Seller{}).
		Where(SellerFilterExp, username).
